...
export class ContextSubst extends LookupSubtable {
    public static read(byte_ar: ByteArray, offset: number, gsub: GsubTable): ContextSubst | null {
        ...
        if (format === 1) return new ContextSubstFormat1(byte_ar, offset, gsub);
        if (format === 2) return new ContextSubstFormat2(byte_ar, offset, gsub);
        if (format === 3) return new ContextSubstFormat3(byte_ar, offset, gsub);
        return null;
    }
}

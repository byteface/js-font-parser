    ...
    function PostTable(de, byte_ar) {
        ...
        if (this.version === 0x00020000) {
            ...
            if (h > 257) {
                ...
                this.psGlyphName = [];
                for (var j = 0; j < h; j++) {
                    var len = byte_ar.readUnsignedByte(); // Read the length of the glyph name
                    var glyphBytes = new Uint8Array(len); // Create a buffer of the appropriate length
                    for (var i = 0; i < len; i++) {
                        glyphBytes[i] = byte_ar.readByte(); // Read each byte into the buffer
                    }
                    var glyphName = new TextDecoder('utf-8').decode(glyphBytes); // Decode the byte array into a string

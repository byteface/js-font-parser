...
export class TrueTypeHintVM {
    runPrograms(glyph, programs, options = {}) {
        const state = {
            stack: [],
            opCount: 0,
            ...
            singleWidthCutIn: 0,
            singleWidthValue: 0,
            ppem: Number.isFinite(options.ppem) ? Math.max(1, Math.round(options.ppem)) : 16,
            autoFlip: true,
            loop: 1,
            rp0: 0,
            rp1: 0,
        ...
        while (ip < program.length) {
            ...
            if (opcode === 0x40) {
                ...
                for (let i = 0; i < count; i++)
                    state.stack.push((program[ip + 2 + i] ?? 0) & 0xff);
                ip += 2 + count;
                continue;
            }
            if (opcode === 0x41) {
            ...
            if (opcode >= 0xb0 && opcode <= 0xb7) {
                ...
                for (let i = 0; i < count; i++)
                    state.stack.push((program[ip + 1 + i] ?? 0) & 0xff);
                ip += 1 + count;
                continue;
            }
            ...
            if (opcode === 0x3e || opcode === 0x3f) {
                ...
                if ((opcode & 1) === 1)
                    target = this.roundByMode(target, state.roundMode, state);
                this.movePointAlongFreedom(state, state.zp0, pointIndex, target - current);
                state.rp0 = pointIndex;
                state.rp0Zone = state.zp0;
                state.rp1 = pointIndex;
    ...
    findEndf(program, start) {
        for (let i = start; i < program.length; i++) {
            if (((program[i] ?? 0) & 0xff) === 0x2d)
                return i;
        }
        return -1;
    }
    seekElseOrEif(program, start) {
        let depth = 0;
        for (let i = start; i < program.length; i++) {
    ...
    getPointAxis(state, zone, pointIndex, axis) {
        const p = this.getZonePoint(state, zone, pointIndex);
        if (!p)
            return 0;
        return axis === 'x' ? p.x : p.y;
    }
    getPointAxisOriginal(state, zone, pointIndex, axis) {
    ...
    getReferenceDelta(state, zone, pointIndex) {
    ...
    movePointAlongFreedom(state, zone, pointIndex, delta) {
        const p = this.getZonePoint(state, zone, pointIndex);
        if (!p || !Number.isFinite(delta))
            return;
    ...
    roundByMode(value, mode, state) {
        ...
        if (mode === 'super')
            return this.roundSuper(value, state);
        return Math.round(value);
    }
    getCvt(state, index) {
        if (index < 0 || index >= state.cvt.length)
            return 0;
        const v = state.cvt[index];
        return Number.isFinite(v) ? v : 0;
    }
    setCvt(state, index, value) {
        ...
        if (idx >= state.cvt.length) {
            ...
            for (let i = 0; i < state.cvt.length; i++) {
                if (!Number.isFinite(state.cvt[i]))
                    state.cvt[i] = 0;
            }
        }
        state.cvt[idx] = Number.isFinite(value) ? value : 0;
    }
    getStorage(state, index) {
        if (!Number.isFinite(index) || index < 0)
            return 0;
        const idx = Math.floor(index);
    ...
    setStorage(state, index, value) {
        ...
        if (idx >= state.storage.length) {
            ...
            for (let i = 0; i < state.storage.length; i++) {
                if (!Number.isFinite(state.storage[i]))
                    state.storage[i] = 0;
            }
        }
        state.storage[idx] = Number.isFinite(value) ? value : 0;
    }
    clampIp(value, length) {
        if (!Number.isFinite(value))
            return length;
        if (value < 0)
            return 0;
    ...
    moveRelativePoint(state, pointIndex, cvtIndex, opcode, pointZone) {
        const refValue = this.getPointAxis(state, state.rp0Zone, state.rp0, state.projectionAxis);
        ...
        if (roundFlag)
            targetAbsDistance = Math.abs(this.roundByMode(targetAbsDistance, state.roundMode, state));
        if (minDistFlag)
            targetAbsDistance = Math.max(state.minDistance, targetAbsDistance);
    ...
    getZonePoint(state, zone, pointIndex) {
    ...
    setPointOnCurve(state, zone, pointIndex, onCurve) {
        const p = this.getZonePoint(state, zone, pointIndex);
        if (!p)
            return;
        p.onCurve = onCurve;
    }
    flipPointOnCurve(state, zone, pointIndex) {
        const p = this.getZonePoint(state, zone, pointIndex);
        if (!p)
            return;
        p.onCurve = !p.onCurve;
    }
    getZoneOriginalPoint(state, zone, pointIndex) {
        if (!Number.isFinite(pointIndex) || pointIndex < 0)
            return null;
        const idx = Math.floor(pointIndex);
        if (zone === 1)
            return state.originalPoints[idx] ?? null;
    ...
    collectContours(points) {
        ...
        if (start < points.length) {
            const contour = [];
            for (let p = start; p < points.length; p++)
                contour.push(p);
            if (contour.length > 0)
                contours.push(contour);
        }
    ...
    interpolateIupDelta(oa, ob, da, db, op) {
        if (!Number.isFinite(oa) || !Number.isFinite(ob) || !Number.isFinite(op))
            return 0;
    ...
    clearTouchedPoint(state, zone, pointIndex) {
        if (!Number.isFinite(pointIndex) || pointIndex < 0)
            return;
        const idx = Math.floor(pointIndex);
        const touchedX = this.getTouchedAxis(state, zone, 'x');
        const touchedY = this.getTouchedAxis(state, zone, 'y');
        while (touchedX.length <= idx)
            touchedX.push(false);
        while (touchedY.length <= idx)
            touchedY.push(false);
        touchedX[idx] = false;
        touchedY[idx] = false;
    }
    applyDeltaPair(state, opcode, target, arg) {
        const decoded = this.decodeDeltaArg(state, opcode, arg);
        if (!decoded || decoded.ppem !== state.ppem)
            return;
    ...
    decodeDeltaArg(state, opcode, arg) {
        ...
        let bandOffset = 0;
        if (opcode === 0x71 || opcode === 0x74)
            bandOffset = 16;
        if (opcode === 0x72 || opcode === 0x75)
            bandOffset = 32;
    ...
    interpolatePointByRefs(state, zone, pointIndex) {
